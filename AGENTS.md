...
- Keep `QLSupportedContentTypes` in `QuickLookExtension/Info.plist`. Do not assume `UTImportedTypeDeclarations` or `UTExportedTypeDeclarations` must live in the host app bundle; verify the effective bundle placement with explicit `UTType` and Quick Look experiments and record the result in `docs/uti.md`.
